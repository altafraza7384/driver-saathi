...
export default function RemindersPage() {
  ...
  const { data: reminders = [], isLoading } = useQuery({
    queryKey: ["reminders"],
    queryFn: async () => { const { data, error } = await supabase.from("reminders").select("*").order("reminder_date", { ascending: true }); if (error) throw error; return data; },
    enabled: !!user,
  });
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ id, completed }: { id: string; completed: boolean }) => { const { error } = await supabase.from("reminders").update({ is_completed: completed }).eq("id", id); if (error) throw error; },
    ...
    <div className="space-y-5 p-4 pt-6">
      ...
      {isLoading ? (
        ...
        <>
          {pending.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-muted-foreground">{t("common.pending")} ({pending.length})</h2>
              <div className="space-y-2">
                {pending.map((r) => (
                  <Card key={r.id} className="border-warning/20"><CardContent className="flex items-center justify-between p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-sm font-medium">{r.title}</p><p className="text-xs text-muted-foreground">{r.reminder_date} • <span className="capitalize">{r.category}</span></p></div>
                    </div>
          ...
          {completed.length > 0 && (
            <section>
              <h2 className="mb-2 text-sm font-semibold text-muted-foreground">{t("common.completed")} ({completed.length})</h2>
              <div className="space-y-2">
                {completed.map((r) => (
                  <Card key={r.id} className="opacity-60"><CardContent className="flex items-center justify-between p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-sm font-medium line-through">{r.title}</p><p className="text-xs text-muted-foreground">{r.reminder_date}</p></div>
                    </div>
                    <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(r.id)}><Trash2 className="h-4 w-4 text-muted-foreground" /></Button>
                  </CardContent></Card>
                ))}
          ...
          {reminders.length === 0 && <Card><CardContent className="p-6 text-center text-muted-foreground">{t("reminder.noReminders")}</CardContent></Card>}
        </>
      )}
    </div>
  );
